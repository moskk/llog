...
public:
    ...
    inline bit_mask& operator = (const bit_mask& v)
    {
        val = v.val;
        return *this;
    }

    inline bit_mask& operator = (const value_t& v)
    {
        val = v;
        return *this;
    }

    // новый объект, в котором
    // установлены все установленные биты из переданного поля
    inline bit_mask operator + (const bit_mask& v) const
    {
        bit_mask bm(*this);
        bm << v;
        return bm;
    ...
    // новый объект, в котором
    // сброшены все биты, установленные в переданном поле
    inline bit_mask operator - (const bit_mask& v) const
    {
        bit_mask bm(*this);
        bm >> v;
    ...
    inline bool operator () (const bit_mask& v) const
    {
        return val&v.val;
    }

    // установлен ли данный бит
    inline bool operator () (value_t v) const
    {
        return val&v;
    }
